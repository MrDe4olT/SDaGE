...
class RightCorridorScene(SceneBase):
    def __init__(self, game) -> None:
        ...
        self.boss_far_image = self.game.assets.images["boss_right_far"]
        self.boss_mid_image = self.game.assets.images["boss_right_mid"]
        self.boss_near_image = self.game.assets.images["boss_right_near"]
    ...
    def draw(self, screen) -> None:
        """Draw the right corridor, boss if visible, and exit button."""
        screen.blit(self.corridor_image, (0, 0))

        office = self.game.session.office
        if office.boss_visible and office.boss_side == "right":
